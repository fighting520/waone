...
class SuperLinksController extends AddonsController{
	...
	public function add(){
		...
		$current = U('/addons/adminlist/name/SuperLinks');
	...
	public function forbidden(){
		...
		if(D('Addons://SuperLinks/SuperLinks')->forbidden($id)){
			$this->success('成功禁用该友情连接', U('/addons/adminlist/name/SuperLinks'));
		}else{
	...
	public function off(){
		...
		if(D('Addons://SuperLinks/SuperLinks')->off($id)){
			$this->success('成功启用该友情连接', U('/addons/adminlist/name/SuperLinks'));
		}else{
			$this->error(D('Addons://SuperLinks/SuperLinks')->getError());
		}
	}
	...
	public function del(){
		...
		if(D('Addons://SuperLinks/SuperLinks')->del($id)){
			$this->success('删除成功', U('/addons/adminlist/name/SuperLinks'));
		}else{
			$this->error(D('Addons://SuperLinks/SuperLinks')->getError());
		}
	}
	...
	public function update(){
		...
		if(!$res){
			...
			if($res['id']){
				$this->success('更新成功', U('/addons/adminlist/name/SuperLinks'));
			}else{
				$this->success('新增成功', U('/addons/adminlist/name/SuperLinks'));
			}
		}
	}
}
