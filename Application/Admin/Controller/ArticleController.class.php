...
class ArticleController extends AdminController {
    ...
    /* 保存允许访问的公共方法 */
    static protected $allow = array( 'draftbox','mydocument','view');
    ...
    protected function getMenu(){
        ...
        //是否展开分类
        $hide_cate = false;
        if(ACTION_NAME != 'recycle' && ACTION_NAME != 'draftbox' && ACTION_NAME != 'mydocument' && ACTION_NAME !='notpass' && ACTION_NAME !='add'){
            $hide_cate  =   true;
        }
        ...
        //获取草稿箱权限
        $this->assign('show_draftbox',  IS_ROOT || $this->checkRule('Admin/article/autoSave'));
        //获取审核列表权限
        $this->assign('show_examine', IS_ROOT || $this->checkRule('Admin/article/examine'));
        
        //获取新增权限
        $this->assign('show_add', IS_ROOT || $this->checkRule('Admin/article/add'));
        
        
        //获取未通过权限
        $this->assign('show_notpass',  IS_ROOT || $this->checkRule('Admin/article/notpass'));
    }
    ...
    public function index($cate_id = null, $model_id = null, $position = null,$group_id=null){
        ...
        $Tjdocument = D("Tjdocument");
      
        //处理列表数据
        if(is_array($list)){
        	foreach ($list as $k=>&$v){
        		$data = $Tjdocument->field('did,sum(view) as view,sum(zits) as zits')->where('did='.$v['id'])->find();
        		$v['username']      =   get_nickname($v['uid']);
        		$v['view'] = $data['view'];
        		if($v['category_id']==543){
        			$Tjmusic = D("Tjmusic");
        			$Resource = D('Resource');
        			$Yueren = M('DocumentYueren');
        			$code = $Yueren->getFieldById($v['id'],'musiccode');
        			if(!empty($code)){
	        			$rid = $Resource->getFieldByResourcecode($code,'id');
	        			$Model2 = new \Think\Model() ;
	        			$wwhits = $Model2->query("select sum(nhits) as sthits from tj_music where rid = ".$rid." and platform like '%ww%' ");
	        			
	        			$yyhits = $Model2->query("select sum(nhits) as sthits from tj_music where rid = ".$rid." and platform like '%yy%' ");
	        			$v['wwhits'] = $wwhits[0]['sthits'];
	        			$v['yyhits'] = $yyhits[0]['sthits'];
        			}
    ...
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null){
        ...
        $list = $this->lists($Document,$map,'position DESC,level DESC,DOCUMENT.id DESC',$field);
    ...
    public function setStatus($model='Document'){
        ...
        if($data['status']==2||!empty($reason)){
    		...
    		if($status == 4){
    			$member = M('Member')->where(array('uid'=>$data['uid']))->find();
    			 M('Member')->where('uid='.$data['uid'])->setDec('artnum');
    			$res = sendmail('lipeng@wawa.fm',"挖哇--《".$data['title']."》审核驳回","您在挖哇上编写的文章《".$data['title']."》没有通过审核：".$reason."<br>请通过以下地址 ".C(WEB_URL)."/waone/admin.php?s=/Public/login.html进行修改操作。<br>谢谢您的支持。");
    			
    		}else{
    			$Member = D('Member');
    			$Member->where('uid='.$data['uid'])->setInc('artnum',1);
    		}
    	//	$logic->checkModelAttr($data['model_id']);
    		$logic->where('id='.$id)->save($data); // 根据条件更新记录
    ...
    public function update(){
        ...
        if(!$res){
        	...
        	if($examine==-30){
        		...
        		$this->success('审核通过', Cookie('__forward__'));
        	}
...
//      			            }
    	...
    	if(is_array($list)){
    		foreach ($list as $k=>&$v){
    			$v['username']      =   get_nickname($v['uid']);
    			$data = $Document->detail($v['id']);
    			$v['reason'] = $data['reason'];
    			$v['audit_time'] = $data['audit_time'];
    		}
    	}
    ...
    public function recycle(){
        ...
        $this->assign('show_clear', IS_ROOT || $this->checkRule('Admin/article/clear'));
        $this->assign('list', $list);
        $this->meta_title       =   '回收站';
        $this->display();
    }
    ...
    public function mydocument($status = null, $title = null){
        ...
        $info['document_pass']	=	$Document ->where('uid='.UID.' and  status =1 ')->count();
                
         
        
        $info['document_my'] = $Document->where('uid='.UID.' and status  in (1,2) ')->count();
        
         
        $info['document_wait'] =$Document ->where('uid='.UID.' and    status =2 ')->count();
