...
class AdminController extends Controller {
    ...
    protected function resume (  $model , $where = array() , $msg = array( 'success'=>'状态恢复成功！', 'error'=>'状态恢复失败！')){
        $data    =  array('status' => 1);
        $examine    =   I('request.examine');
        if($model=="Document"&&$examine=="-30"){
    ...
    protected function restore (  $model , $where = array() , $msg = array( 'success'=>'状态还原成功！', 'error'=>'状态还原失败！')){
        ...
        if($model=="Document"){
        	...
        	foreach ($ids as $id)
        	{
        		...
        		$tempstatus = $Document->where('id='.$id)->getField('tempstatus');
        
        		$data    =  array('status'=>$tempstatus);
        		// 构建列表数
        
        		$isw =  $Document-> where('id='.$id)->setField($data);
    ...
    protected function delete ( $model , $where = array() , $msg = array( 'success'=>'删除成功！', 'error'=>'删除失败！')) {
        ...
        if($model=="Document"){
        	...
        	foreach ($ids as $id)
        	{
        		...
        		$tempstatus = $Document->where('id='.$id)->getField('status');
        		
        		$data    =  array('status' =>-1,'tempstatus'=>$tempstatus);
        		// 构建列表数		
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function reject ( $model , $where = array() , $msg = array( 'success'=>'驳回成功！', 'error'=>'驳回失败！')) {
    	$data['status']         =   4;
    	$this->editRow(   $model , $data, $where, $msg);
    }
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function draft ( $model , $where = array() , $msg = array( 'success'=>'驳回成功！', 'error'=>'驳回失败！')) {
    	$data['status']         =   3;
    	$this->editRow(   $model , $data, $where, $msg);
    }
    ...
    public function setStatus($Model=CONTROLLER_NAME){
        ...
        switch ($status){
           ...
           case 3  :
                ...
                $this->draft($Model, $map, array('success'=>'撤回成功','error'=>'撤回失败'));
                	break;
            case 4  :
                $this->reject($Model, $map, array('success'=>'驳回成功','error'=>'驳回失败'));
                break;
            default :
                $this->error('参数错误');
                break;
        	...
        	if($group[0]['group_id']==2){
        		...
        		$g='个人信息';
        		  $map = array('group'=>$g);
                            ...
                            $menuList = M('Menu')->where($map)->field('id,pid,title,url,tip')->order('sort asc')->select();
                            $menus['child'][$g] = list_to_tree($menuList, 'id', 'pid', 'operater', 16);
        	}
      }
        return $menus;
    }
...
//sql 分页查询
    public function sqllists ($sql,$db2) {
    	$Model = new \Think\Model(); // 实例化一个model对象 没有对应任何数据表
    	if(!empty($db2)){
    	  $Model = $Model->db(2,'DB_CONFIG2');
    	}
    	$countSqlString = "select count(*) as total ".$this->removeSelect($sql);
       
    	$REQUEST    =   (array)I('request.');
    	
    	$count =  $Model->query($countSqlString);
        $total        =  count($count);
     ...
     function removeSelect($qlString){
    	$beginPos = strpos(strtolower($qlString),"from");
         
    	return  substr($qlString,$beginPos);
    }
