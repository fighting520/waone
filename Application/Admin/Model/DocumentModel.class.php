...
class DocumentModel extends Model{
    ...
    protected $_validate = array(
       ...
       // array('bsname', 'checkName', '标识已经存在', self::VALUE_VALIDATE, 'callback', self::MODEL_BOTH),
        array('title', 'require', '标题不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
        array('title', '1,40', '标题长度不能超过40个字符', self::MUST_VALIDATE, 'length', self::MODEL_BOTH),
        array('level', '/^[\d]+$/', '优先级只能填正整数', self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
        array('description', '1,80', '简介长度不能超过80个字符', self::VALUE_VALIDATE, 'length', self::MODEL_BOTH),
    ...
    public function update($data = null){
        ...
        if(empty($data['id'])){ //新增数据
        	...
        	if($examine==-20){
        		...
        		$res = sendmail('lipeng@wawa.fm',"挖哇--《".$data['title']."》审核驳回","您在挖哇上编写的文章《".$data['title']."》没有通过审核："."<br>请通过以下地址 ".C(WEB_URL)."/waone/admin.php?s=/Public/login.html进行修改操作。<br>谢谢您的支持。");
        		$data['auid'] = UID;
    ...
    public function remove(){
        ...
        if ( is_administrator() ) {
            ...
            $map = array('status'=>-1,'category_id'=>array( 'IN',trim(implode(',',$cate_ids),',') ),'id'=>array( 'IN',$ids ));
        }
    ...
    public function autoSave(){
        ...
        //触发自动保存的字段
        $save_list = array('name','title','description','position','link_id','cover_url','deadline','create_time','content');
        ...
        $this->_validate = array(
         ...
         //   array('bsname', '', '标识已经存在', self::VALUE_VALIDATE, 'unique', self::MODEL_BOTH),
