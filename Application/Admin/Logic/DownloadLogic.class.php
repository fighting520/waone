...
class DownloadLogic extends BaseLogic{
    ...
    protected $_validate = array(
        ...
        array('file_url', 'require', '请上传附件！', self::MUST_VALIDATE , 'regex', self::MODEL_BOTH),
    );
    ...
    public function detail($id){
        ...
        $file = D('File')->field(true)->getByFile_url($data['file_url']);
        return $data;
    }
    ...
    public function update($id = 0){
       ...
       // $file = json_decode(think_decrypt(I('post.file_url')), true);
        $file = D('File')->field(true)->getByFile_url($data['file_url']);
        if(!empty($file)){
