...
class UserController extends HomeController {
	...
	/* 注册页面 */
	public function register($username = '', $password = '', $repassword = '', $email = '', $verify = '',$pimg='',$card='',$remark='',$lanmu='',$bqname=''){
		...
		if(IS_POST){ //注册用户
			...
			if(0 < $uid){ //注册成功
				//TODO: 发送验证邮件
				$user = array('uid' => $uid, 'nickname' => $username, 'status' => 2,
						'password' => $password,
						'email'    => $email,
						'mobile'   => $mobile,
						'reg_time' => NOW_TIME,
				...
				$this->success('注册成功！',U('Home/User/success2'));
			} else { //注册失败，显示错误信息
